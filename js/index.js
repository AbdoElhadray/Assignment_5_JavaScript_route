...
async function get3days(location) {
  var fetchData = await fetch(`https://api.weatherapi.com/v1/forecast.json?
...
async function getAll(searchedCity) {
  var data_GetCurrentWeather = await getCurrentWeather(searchedCity);
  // console.log("GetCurrentWeather", data_GetCurrentWeather);
  var today = convertDateToObject(
...
`;
  ...
  var data_get3days = await get3days(searchedCity);
  // console.log("get3days", data_get3days);
  var tomorrow = convertDateToObject(
    data_get3days.forecast.forecastday[1].date
  );
  var afterTomorrow = convertDateToObject(
